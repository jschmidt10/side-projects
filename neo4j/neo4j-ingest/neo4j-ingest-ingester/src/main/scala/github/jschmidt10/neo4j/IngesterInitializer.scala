...
import github.jschmidt10.neo4j.message.KryoMessageSerialization
import github.jschmidt10.neo4j.Ingester
...
class IngesterInitializer {
  ...
  @Parameter(names = Array("-n", "--neo4jUrl"), description = "Neo4J Driver URL (example: \"bolt://localhost:7687\")", required = true)
  var neo4jUrl: String = ""

  // TODO: Authentication needs to be handled somewhere else
  @Parameter(names = Array("-nu", "--neo4jUser"), description = "Neo4J username", required = true)
  var neo4jUser: String = ""

  @Parameter(names = Array("-np", "--neo4jPassword"), description = "Neo4J password", required = true)
  var neo4jPassword: String = ""

  @Parameter(names = Array("-c", "--consumer"), description = "Fully qualified consumer class to use. Defaults to github.jschmidt10.neo4j.KafkaGraphConsumer", required = false)
  ...
  @Parameter(names = Array("-s", "--serialization"), description = "Fully qualified serialization class to use. Defaults to github.jschmidt10.neo4j.message.KryoMessageSerialization", required = false)
  ...
  def createIngester(): Ingester = {
    // TODO: Need mechanism to handle alternate authentication methods.
