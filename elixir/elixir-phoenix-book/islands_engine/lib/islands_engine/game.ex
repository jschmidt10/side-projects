defmodule IslandsEngine.Game do
  ...
  def handle_call({:position_island, player, key, row, col}, _from, state) do
    ...
    with {:ok, rules} <- Rules.check(state.rules, {:position_islands, player}),
      ...
      :error -> reply_msg(state, :error)
      err = {:error, :invalid_coordinate} -> reply_msg(state, err)
      err = {:error, :invalid_island_type} -> reply_msg(state, err)
    end
  end
  ...
  def handle_call({:set_islands, player}, _from, state) do
    ...
    with {:ok, rules} <- Rules.check(state.rules, {:set_islands, player}),
      ...
      :error -> reply_msg(state, :error)
      false -> reply_msg(state, {:error, :not_all_islands_positioned})
    end
  end
  ...
  def handle_call({:guess_coordinate, player, row, col}, _from, state) do
    ...
    do
      ...
      |> update_rules(rules)
      |> reply_msg({hit_or_miss, forested_island, win_status})
    else
      :error -> reply_msg(state, :error)
      error = {:error, :invalid_coordinate} -> reply_msg(state, error)
    end
  end
  ...
  defp update_player2(state, name), do: put_in(state.player2.name, name)

  defp update_rules(state, rules), do: %{state | rules: rules}

  defp reply_msg(state, reply) do
